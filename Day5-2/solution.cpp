...
/* Simply add support for iterators of diagonals, and stop ignoring them in the 
 * main function. */
...
private:
    ...
    private:
        Point2D get_step(const Line& line) {
            ...
            if (line.p1.x - line.p2.x == line.p1.y - line.p2.y) { // Slope up
                if (line.p1.x < line.p2.x)
                    return {1, 1};
                return {-1, -1};
            }
            if (line.p1.x - line.p2.x == -(line.p1.y - line.p2.y)) { // Slope down
                if (line.p1.x < line.p2.x)
                    return {1, -1};
                return {-1, 1};
            }
            
            std::cout << "\n Error in get_step(): We only support horizontal, vertical, or 45 degree diagonal lines \n"; 
            assert(false);
        }
    };
};
